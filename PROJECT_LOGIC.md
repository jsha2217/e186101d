...
- `database.rules.json`: `.read: true`, `.write: true` — 인증 없이 전체 공개/쓰기 가능한 규칙. 보안은 전적으로 클라이언트단 비밀번호 잠금에 의존(사실상 URL/DB endpoint를 아는 사람은 누구나 직접 접근 가능한 구조).

## 3. 실시간 동기화

- `db.ref('contacts').on('value', ...)`, `db.ref('appointments').on('value', ...)` 로 항상 최신 스냅샷을 `contacts`/`appointments` 전역 객체에 반영 후 3개 뷰 모두 재렌더링.
- `setInterval(renderAppointments, 30000)`: 데이터 변경이 없어도 시간 경과에 따라 "임박도"와 "지난 약속 → 보관함" 전환이 자동 반영되도록 30초마다 재계산.

## 4. 화면 구성 (탭 3개, `.view` 토글 + 하단 tabbar)
...
- 각 카드에 수정/삭제 버튼. 삭제 시 확인창 후 해당 연결자와 **연결된 모든 약속도 함께 삭제**.
...
- 등록/수정 모달의 연결자 select는 내가 관련된(메인/서브) 연결자만 옵션으로 노출(`populateContactSelect`).

### 4-3. 전체 연결 현황 탭 (`allContactsView`)
...
- `saveContact`/`saveAppt`에는 클라이언트 측 최소 검증만 존재(이름/연결자 선택 필수) — 그 외 필드는 전부 선택.
